...
AST::NodeList parseMatchArmBody(Parser& parser) {
...
// `match <subject> { Variant(a, b) => body, Unit => body, _ => body }`.
...
AST::NodePtr Parser::parseMatch() {
    const Token& start = current();
    advance();  // consume `match`

    auto node = std::make_shared<AST::MatchStatement>();
    node->subject = ecxParseCondition(*this);

    skipNewlines();
    expect(TokenType::LBrace, "E1460", "'{' to begin match body");
    ...
    while (!atEnd() && !check(TokenType::RBrace)) {
        ...
        if (check(TokenType::Identifier) && current().value == "_") {
            ...
            if (check(TokenType::LParen)) {
                ...
                expect(TokenType::RParen, "E1462", "')' to close match bindings");
                match(TokenType::RParen);
            }
        } else {
            error("E1463", "expected a variant name or '_' in match arm",
                  "match arms look like `Variant(bindings) => body`");
            break;
        }

        expect(TokenType::FatArrow, "E1464", "'=>' after match pattern");
        match(TokenType::FatArrow);
        skipNewlines();

        arm.body = parseMatchArmBody(*this);
    ...
    expect(TokenType::RBrace, "E1465", "'}' to close match body");
